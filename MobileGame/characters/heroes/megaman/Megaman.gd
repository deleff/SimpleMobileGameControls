...
func _on_enemy_jabbed(hero, enemy):
	if hero == (self):
		...
		if state.state_name() == "NeutralState":
			change_state("jab_1")
			jab_window.start(0.3)
		elif state.state_name() == "Jab_1_State" && jab_window.time_left != 0:
			change_state("jab_2")
			jab_window.start(0.3)
...
func change_state(new_state_name):
	...
	state.setup(funcref(self, "change_state"), target_position, "megaman", "heroes", $Sprite, self, currently_attacking)
	add_child(state)
