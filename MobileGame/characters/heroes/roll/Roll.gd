...
func _ready():
	...
	signal_message_queue.connect("hit", self, "_on_roll_hit")
	...
	var start_sound = load("res://characters/heroes/roll/sfx/instantiated.wav")
	audio.stream = start_sound
	audio.play()

func _on_roll_hit(from, to, attack_type, amount_of_damage, hit_direction):
