...
func _ready():
	...
	if persistent_state.position.x - target_position.x < 0:
		sprite.scale.x = 1
		rolling_target_position = Vector2((persistent_state.position.x + 150),persistent_state.position.y)
	else:
		sprite.scale.x = -1
		rolling_target_position = Vector2((persistent_state.position.x - 150),persistent_state.position.y)

func _physics_process(_delta):
	velocity = persistent_state.position.direction_to(rolling_target_position) * speed
	persistent_state.velocity = (velocity)
	if persistent_state.position.distance_to(rolling_target_position) <= 5:
